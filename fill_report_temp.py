...
def set_run_font(run, font_name_cn='宋体', font_name_en='Times New Roman', size=Pt(12), bold=False, color=None):
    run.font.size = size
    run.font.bold = bold
...
def set_para_fmt(para, first_indent=Cm(0.74), line_spacing=18, align=None):
...
def insert_after(para, text, style_name='Normal'):
    """Insert a new paragraph after `para` with given text."""
    new_p = OxmlElement('w:p')
    para._element.addnext(new_p)
    p_obj = type(para)(new_p, para._parent)
    p_obj.style = doc.styles[style_name]
    run = p_obj.add_run(text)
    return p_obj

def add_normal(text, after, indent=Cm(0.74)):
    p = insert_after(after, text)
    set_para_fmt(p, first_indent=indent)
    for run in p.runs:
        set_run_font(run)
    return p

def add_code(text, after):
    p = insert_after(after, text)
    set_para_fmt(p, first_indent=None)
    for run in p.runs:
        set_run_font(run, font_name_cn='宋体', font_name_en='Consolas', size=Pt(10))
...
# ==================== 2. Main title para [29] ====================
p29 = doc.paragraphs[29]
p29.clear()
run = p29.add_run('基于电商商品数据集的商品特征及价格行为分析')
run.font.size = Pt(16)
run.font.bold = True
...
p29.alignment = WD_ALIGN_PARAGRAPH.CENTER

# ==================== 3. 概述 [31] ====================
p31 = doc.paragraphs[31]
p31.clear()
run = p31.add_run('本次课程设计旨在掌握 Python 数据分析的全流程。通过对公开电商商品数据集进行系统性的分析，完成了从数据获取、预处理、探索性分析到可视化展示的闭环工作。本次分析旨在通过数据手段揭示商品类别、定价策略与用户评分之间的潜在规律，并为电商平台的运营提供数据支持。')
set_run_font(run)
set_para_fmt(p31)
...
p37.clear()
run = p37.add_run('本实验采用网络请求技术，从 DummyJSON 开放平台（https://dummyjson.com/products）获取了原始的商品数据集。该数据集包含了 100 条商品记录，字段涵盖了 id（唯一标识）、title（商品名称）、category（商品类别）、price（价格）、discountPercentage（折扣百分比）、rating（用户评分）、stock（库存数量）及 brand（品牌）。数据呈现出明显的电商业务特征，且包含部分缺失值与异常点，非常适合进行数据清洗与挖掘练习。')
set_run_font(run)
set_para_fmt(p37)

p38 = doc.paragraphs[38]
p38.clear()
run = p38.add_run('以下为数据获取的核心代码：')
set_run_font(run)
set_para_fmt(p38)
...
p_c1 = add_code(code1, after=p38)
p_d1 = add_normal('通过上述代码，成功获取了 100 条商品记录，包含 8 个核心字段，为后续分析奠定了基础。', after=p_c1)

# ==================== 5. 数据处理 [40] ====================
p40 = doc.paragraphs[40]
p40.clear()
run = p40.add_run('为确保分析结果的科学性，对原始数据进行了深度清洗，具体处理流程如下：')
set_run_font(run)
set_para_fmt(p40)
...
for t in [
    ...
    p = add_normal(t, after=prev)
    prev = p

p_pi = add_normal('数据处理的实现代码如下：', after=prev)
...
p_c2 = add_code(code2, after=p_pi)
p_pr = add_normal('经过上述清洗步骤，数据质量显著提升，为后续分析提供了可靠的数据基础。', after=p_c2)

# ==================== 6. 数据分析 [43] ====================
p43 = doc.paragraphs[43]
p43.clear()
run = p43.add_run('在清洗后的数据基础上，主要从以下两个维度进行了深入分析：')
set_run_font(run)
set_para_fmt(p43)
...
p_ac = add_normal('数据分析的核心代码如下：', after=prev)
...
p_c3 = add_code(code3, after=p_ac)
p_ar = add_normal('通过分析发现，不同商品类别的平均价格存在显著差异，电子产品和高端家居类目远高于日用百货。同时，价格与评分的相关系数接近零，说明价格高低并不直接影响用户评分。', after=p_c3)

# ==================== 7. 数据可视化 [45] ====================
p45 = doc.paragraphs[45]
p45.clear()
run = p45.add_run('通过 Matplotlib 和 Seaborn 库，构建了四种不同类型的图表进行直观呈现。')
set_run_font(run)
set_para_fmt(p45)
...
p_vc = add_normal('数据可视化核心代码如下：', after=prev)
...
p_c4 = add_code(code4, after=p_vc)

# ==================== 8. 结论与总结 [48] ====================
p48 = doc.paragraphs[48]
p48.clear()
run = p48.add_run('通过本次分析，得出以下核心结论：')
set_run_font(run)
set_para_fmt(p48)
...
# ==================== 9. Remove red instruction [53] ====================
doc.paragraphs[53].clear()
