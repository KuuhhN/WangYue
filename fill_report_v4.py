"""
v4: Use low-level XML manipulation (addnext) instead of insert_paragraph_before.
Process from BOTTOM to TOP to avoid position shift issues.
Build content as XML elements and insert them at the right place.
"""
...
def make_paragraph(text, is_code=False, indent=True):
    """Create a w:p element with w:r, w:t, and formatting."""
    p = OxmlElement('w:p')
    pPr = OxmlElement('w:pPr')
    
    # Line spacing (fixed 18pt = 360 twips... actually 18pt = 18*20 = 360)
    ...
    # First line indent: 2 chars = ~480 twips (0.74cm ≈ 420 twips)
    if indent and not is_code:
...
def insert_paragraphs_before(paragraphs, before_heading_text):
    """Insert list of paragraph XML elements before a heading."""
    # Find the heading paragraph element
    heading_elem = None
    for p in doc.paragraphs:
        if p.text.strip() == before_heading_text and p.style.name.startswith('Heading'):
            heading_elem = p._element
            break
    
    if heading_elem is None:
        print(f'  ERROR: could not find heading "{before_heading_text}"')
        return False
    
    # Insert in reverse order so they appear in correct reading order
    for para_xml in reversed(paragraphs):
        heading_elem.addprevious(para_xml)
    
    return True

# ==============================
# STEP 1: Update cover table and main title
# ==============================
table0 = doc.tables[0]
...
# Update main title
for p in doc.paragraphs:
    if '××' in p.text and p.style.name.startswith('Heading'):
        p.clear()
...
# ==============================
# STEP 2: Delete placeholder and red-text paragraphs
# ==============================
to_delete_texts = [
    '介绍本次课程实际完成的任务',
...
for p in doc.paragraphs:
    t = p.text.strip()
    if t in to_delete_texts:
        to_delete.append(p)

for p in reversed(to_delete):
    p._element.getparent().remove(p._element)
print(f'Deleted {len(to_delete)} paragraphs')

# ==============================
# STEP 3: Build content for each section and insert from bottom to top
# ==============================

# --- 结论与总结 content (insert before 评语) ---
conc_texts = [
    '通过本次分析，得出以下核心结论：',
...
conc_paras = [make_paragraph(t) for t in conc_texts]
if insert_paragraphs_before(conc_paras, '评语'):
    print('结论与总结: OK')

# --- 数据可视化 content (insert before 结论与总结) ---
vis_texts = [
...
vis_all = []
for t in vis_texts:
    vis_all.append(make_paragraph(t))
vis_all.append(make_paragraph('数据可视化核心代码如下：', indent=False))
vis_all.append(make_paragraph(vis_code, is_code=True))

if insert_paragraphs_before(vis_all, '结论与总结'):
    print('数据可视化: OK')

# --- 数据分析 content (insert before 数据可视化) ---
ana_texts = [
...
ana_all = []
for t in ana_texts:
    ana_all.append(make_paragraph(t))
ana_all.append(make_paragraph('数据分析的核心代码如下：', indent=False))
...
if insert_paragraphs_before(ana_all, '数据可视化'):
    print('数据分析: OK')

# --- 数据处理 content (insert before 数据分析) ---
proc_texts = [
...
proc_all = []
for t in proc_texts:
    proc_all.append(make_paragraph(t))
proc_all.append(make_paragraph('数据处理的实现代码如下：', indent=False))
proc_all.append(make_paragraph(proc_code, is_code=True))
proc_all.append(make_paragraph('经过上述清洗步骤，数据质量显著提升，为后续分析提供了可靠的数据基础。'))

if insert_paragraphs_before(proc_all, '数据分析'):
    print('数据处理: OK')

# --- 数据获取 content (insert before 数据处理) ---
...
get_all = [
    ...
    make_paragraph('以下为数据获取的核心代码：', indent=False),
    make_paragraph(get_code, is_code=True),
    make_paragraph('通过上述代码，成功获取了 100 条商品记录，包含 8 个核心字段，为后续分析奠定了基础。'),
]

if insert_paragraphs_before(get_all, '数据处理'):
    print('数据获取: OK')

# --- 概述 content (insert before 数据获取) ---
...
if insert_paragraphs_before([make_paragraph(overview_text)], '数据获取'):
    print('概述: OK')
...
for i, p in enumerate(doc2.paragraphs):
    ...
    if text:
        ...
        style = p.style.name[:10]
        print(f'[{i:3d}] [{style:10s}] {display}')
